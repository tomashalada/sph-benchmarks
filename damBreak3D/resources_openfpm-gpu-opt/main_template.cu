...
const real_number dp = placeholderInitParticleDistance;
...
const real_number H = placeholderSmoothingLength;
...
const real_number MassFluid = placeholderMass;

// Mass of the boundary particles
const real_number MassBound = placeholderMass;
...
__global__ void verlet_int_gpu(vector_dist_type vd, real_number dt, real_number dt2, real_number dt205)
{
    ...
    if (vd.getPos(a)[0] <  0.0 || vd.getPos(a)[1] < 0.0 || vd.getPos(a)[2] < 0.0 ||
        vd.getPos(a)[0] >  3.22 || vd.getPos(a)[1] > 1.0 || vd.getPos(a)[2] > 1.5 ||
		vd.template getProp<rho>(a) < RhoMin || vd.template getProp<rho>(a) > RhoMax)
    {
...
int main(int argc, char* argv[])
{
	...
	size_t sz[3] = {placeholderGridXSize,placeholderGridYSize,placeholderGridZSize};
